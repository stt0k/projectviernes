...
@Component({
  ...
  template: `
    <div class="p-6 max-w-6xl mx-auto min-h-screen">
      <h2 class="text-2xl font-semibold text-white text-center mb-8">Gestión de Plantilla</h2>

      <div class="mb-6 flex flex-col sm:flex-row gap-4 items-center justify-center">
        <select
          [(ngModel)]="funcionSeleccionada"
          class="bg-zinc-800/30 border border-white/10 rounded px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-white/20"
        >
          <option value="" class="bg-zinc-800 text-white">Seleccione una función</option>
          @for (funcion of funciones; track funcion) {
          <option [value]="funcion" class="bg-zinc-800 text-white">{{ funcion }}</option>
          }
        </select>
...
export class Plantilla implements OnInit {
  ...
  async mostrarPlantilla() {
    this.plantillas = await this.service.getPlantillaFuncion(this.funcionSeleccionada);
  }
}
